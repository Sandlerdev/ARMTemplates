...
resource twinUpdateServicelistenauthrules 'Microsoft.EventHub/namespaces/eventhubs/authorizationRules@2021-11-01' ={
  name: '${EventHubNS.name}/telemetrydata/TwinUpdateService'
  dependsOn: [
    telemetrydata
  ]
  properties:{
...
/*Twin Parser*/
resource ITDTwinParser 'Microsoft.Web/sites@2021-03-01' = {
  name: '${ResourcePrefix}ITDTwinParser'
  kind: 'functionapp,linux,container'
  location: location
  ...
  properties: {
   enabled: true
    ...
    hostNameSslStates: [
      {
        name: '${ResourcePrefix}itdtwinparser.azurewebsites.net'
        sslState: 'Disabled'
        hostType: 'Standard'
      }
      {
        name: '${ResourcePrefix}itdtwinparser.scm.azurewebsites.net'
        sslState: 'Disabled'
        hostType: 'Repository'
      }
    ]
...
resource twinParserSettings 'Microsoft.Web/sites/config@2018-11-01' = {
  name: 'appsettings'
  kind: 'string'
  parent: ITDTwinParser
  properties: {
    AzureWebJobsStorage: blobStorageConnectionString
    DeleteOnProcessComplete: 'true'
    DiagnosticServices_EXTENSION_VERSION: '~3'
    adtinstanceUrl:'https://${adtInstance.name}.api.eus.digitaltwins.azure.net'
    TwinUpdateServiceEventHub: twinUpdateServicelistenauthrules.listKeys().primaryConnectionString
    FUNCTIONS_EXTENSION_VERSION: '~3'
    WEBSITE_CONTENTAZUREFILECONNECTIONSTRING: blobStorageConnectionString
    WEBSITE_CONTENTSHARE: 'itdiotparserb794' 
    WEBSITES_ENABLE_APP_SERVICE_STORAGE: 'false'
...
resource adtInstance 'Microsoft.DigitalTwins/digitalTwinsInstances@2021-06-30-preview' = {
  name: '${ResourcePrefix}ADT'
  location: location
